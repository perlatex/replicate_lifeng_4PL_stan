...
data {
  ...
  // 先验超参数 (由 R 传入, 便于比较 tight / wide 先验)
...
transformed parameters {
  ...
  for (n in 1:N) {
    int j = compound[n];
    mu[n] = response_mean(concentration[n], bottom[j], top[j], ec50[j], hill[j]);
  }
}
model {
  // Explicit priors; log_gap ensures top > bottom.
  // These priors are choices for this reanalysis, not the paper's priors.
  // wide  (bottom_sd = 1,   gap_sd = 1,    hill ~ N(0, 1.5)):        MAP 与 nls / GraphPad 一致.
  // tight (bottom_sd = 0.2, gap_sd = 0.25, hill ~ N(log(1.5), 0.8)): 主要因 bottom 先验,
  //   ent-Paroxol 的 bottom 被拉向 0、hill 抬高, 两条曲线靠拢.
  target += normal_lpdf(bottom | 0, prior_bottom_sd);
  target += normal_lpdf(log_gap | 0, prior_gap_sd);
  target += normal_lpdf(log_ec50 | log(300), 1);
  target += normal_lpdf(log_hill | prior_hill_mu, prior_hill_sd);
...
generated quantities {
  ...
  for (m in 1:M) {
    ...
    mu_grid[m] = response_mean(concentration_grid[m], bottom[j], top[j], ec50[j], hill[j]);
